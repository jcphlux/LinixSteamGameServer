...
- **setup_server.sh**: Automates the setup of Steam game servers on a Linux machine.
  ...
  - Configures SFTP access for updating server files.
...
The setup script configures SFTP access for the service user, allowing you to update server files if needed.

### Steps to Configure SFTP Access:

1. **Generate SSH Key Pair**:
   The setup script generates an SSH key pair for the service user. The public key is displayed at the end of the script execution.

2. **Copy the Public Key**:
   Copy the public key displayed by the script.

3. **Add the Public Key to the Remote Client**:
   Add the copied public key to the `authorized_keys` file on the remote client you want to connect from.

4. **Connect via SFTP**:
   Use an SFTP client to connect to the server using the service user's credentials and the private key.

### Example SFTP Command:

```bash
sftp -i /path/to/private/key $SERVICE_USER@server_ip
```

Replace `/path/to/private/key` with the path to the private key generated by the setup script and `server_ip` with the IP address of your server.
